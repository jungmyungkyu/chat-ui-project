...
- 공급망, 거시경제, 금리 등 원문에 없는 일반적 리스크를 임의로 추가하지 말 것
- 모든 핵심 판단에는 transcript에서 직접 복사한 문장을 인용하고 [^1^] 형태로 본문에 표시
- 최소 4개 이상의 서로 다른 원문 인용을 포함하되,
  하단에 footnote 목록(# 6 근거 주석)은 절대 출력하지 말 것
- footnote 번호 설명 목록은 작성하지 말 것
...
## 기업에 긍정적인 신호
- bullet 3개 이상
- 각 항목은 구체적이어야 하며 원문 인용 footnote 포함
## 기업에 부정적인 신호
- bullet 2~3개
- 단기 실적 압박인지, 구조적 문제인지 구분
- 반드시 transcript에 근거가 있는 내용만 작성
※ 이 섹션에서는 기업 자체의 펀더멘털 관점만 다룬다.

# 3) 전분기 대비 변화
- 가이던스 변화 (상향/하향/유지)
- 성장률 변화
- 마진 변화
- 리스크 언급 증감 (원문 기반으로만 작성)
- 경영진 톤 변화
- 반복 키워드 Top 5

- 한줄요약:
  이번 어닝콜이 투자자 관점에서 갖는 핵심 의미를
  긍정/부정을 모두 반영하여
  균형 있게 한 문장으로 정리하라 (과장 금지, 단정 금지)

# 4) 포트폴리오 영향
## 사용자 포트폴리오 관점
- 반드시 아래 포트폴리오 JSON을 먼저 해석하라.
- 현재 분석 대상 기업이 포트폴리오에 포함되어 있는지 먼저 판단하라.

[분기 규칙]

1. 만약 분석 대상 기업이 포트폴리오에 포함되어 있다면:
   - 보유 비중을 반영하여 직접 영향 분석
   - 행동 레벨 판단
   - 비중이 높을수록 리스크 민감도를 반영

2. 만약 분석 대상 기업이 포트폴리오에 포함되어 있지 않다면:
   - "현재 포트폴리오에 해당 기업은 포함되어 있지 않습니다."라고 명확히 작성
   - 직접 영향은 제한적임을 설명
   - 산업/밸류체인/AI·클라우드·반도체 등 연관성을 기반으로 간접 영향 분석
   - 직접 보유하지 않은 경우 과도한 경고 금지

- 행동 레벨을 반드시 위 4단계 중 하나로 선택하고 굵게 표시
...
- 구조적 훼손 없음 → 🟢 정상 모니터링
- 변화 감지, 방향성 불확실 → 🟡 주의 관찰
- 펀더멘털 변화 신호 발생 → 🟠 전략 재점검
- 하방 리스크 구조적 확대 → 🔴 즉시 점검 필요
